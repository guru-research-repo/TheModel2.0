# Run using CUDA_VISIBLE_DEVICES=2 python martha_cnn_salience.py
import os
import numpy as np
import pandas as pd
import datetime
...
from Datasets_cnn import *

def main(lp=False, dataset_name="salience"):
    os.makedirs("sorted5FACES_CNN_32_fixations_40_epochblock_Martha", exist_ok=True)
    ...
    pd.DataFrame(history).to_csv(
        f"sorted5FACES_CNN_32_fixations_40_epochblock_Martha/training_history_{ts}.csv", index=False
    )
    pd.DataFrame(history_acc).to_csv(
        f"sorted5FACES_CNN_32_fixations_40_epochblock_Martha/overall_training_history_{ts}.csv", index=False
    )

    torch.save(
        model.state_dict(), f"sorted5FACES_CNN_32_fixations_40_epochblock_Martha/resnet18_{ts}.pth"
    )
