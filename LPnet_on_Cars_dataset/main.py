...
# def main():
 ...
 # --- multi‑GPU wrap ---
    ...
    df.to_csv(f"cars_results/training_history_{ts}.csv", index=False)

    torch.save(model.state_dict(), f"cars_results/resnet18_{ts}.pth")
    #####################################################################################################
    return history

def main():
    os.makedirs("cars_results", exist_ok=True)
    ...
    df_all.to_csv("cars_results/training_history_5runs.csv", index=False)
    ...
    summary.to_csv("cars_results/training_summary_avg_std.csv")
    ########################################################################################################
