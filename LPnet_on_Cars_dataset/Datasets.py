...
def load_dataset(dataset, identity=4, task="train_upright", label_mapping=None):
    if dataset == "celeb":
        ...
        ds = CelebrityFacesDataset(root_dir="../kira/cleaned_glabella_processed_data", num_identities=identity, split=task) # label_mapping = label_mapping
    elif dataset == "cars":
        ds = CelebrityCarsDataset(root_dir="cars_processed_data", num_identities=identity, split=task) 
    return ds
