...
for split in ['train', 'valid']: #['test', 'train', 'valid']:
    ...
    save_dir = Path(f'processed_data/salience/updated_faces/{num_identities}_identities/{split}')
    save_dir.mkdir(parents=True, exist_ok=True)

    pipeline = SaliencePipeline(split, logpolar=True, num_salient_points=64) # Create pipeline to transform images.

    # Get directory for the current split
    for label_dir in split_dir.iterdir(): # folder of images for each person
        #Make subdirfor each person in save_dir 
        Path(f'processed_data/salience/updated_faces/{num_identities}_identities/{split}/{label_dir.name}').mkdir(parents=True, exist_ok=True)
        ...
        for img_path in label_dir.iterdir(): # image paths for each person
            ...
            for n, transformed_img_tensor in enumerate(transformed_imgs[0]): #torch.tensor (C,H,W). note: batch_size=1 from unsqueeze above.
                ...
                file_path = f'processed_data/salience/updated_faces/{num_identities}_identities/{split}/{label_dir.name}/{img_path.stem}_proc{n}.png'
