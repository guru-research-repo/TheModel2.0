...
def get_label_mapping(root_dir="deepgaze2_processed_data", num_identities=128, split="valid_inverted"): # was data
    """
...
def label_to_one_hot(label, mapping):
    ...
    # print(f"[label_to_one_hot] Label: '{label}' → Index: {idx} / {num_classes}")
    one_hot = F.one_hot(torch.tensor(idx, dtype=torch.long), num_classes=num_classes).float()
    # print(f"[label_to_one_hot] One-hot: {one_hot}")
    return one_hot
