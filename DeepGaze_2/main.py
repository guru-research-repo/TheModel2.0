...
# def main():
    ...
    total_epochs    = 240
    epoch_block     = 40  # how many epochs per identity ########################################### make it 40
 ...
 # --- multi‑GPU wrap ---
    ...
    df.to_csv(f"deepgaze2_LPNet_results/training_history_{ts}.csv", index=False)

    torch.save(model.state_dict(), f"deepgaze2_LPNet_results/resnet18_{ts}.pth")
    #####################################################################################################
    return history

def main():
    os.makedirs("deepgaze2_LPNet_results", exist_ok=True)
    ...
    df_all.to_csv("deepgaze2_LPNet_results/training_history_5runs.csv", index=False)
    ...
    summary.to_csv("deepgaze2_LPNet_results/training_summary_avg_std.csv")
    ########################################################################################################
