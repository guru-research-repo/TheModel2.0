                ...
                for img_file in label_dir.iterdir():
                    ...
                    for i, tensor in enumerate(crops):
                        ...
                        for folder_name, inversion in targets:
                            ...
                            rotated = foveation(rotated)
                            C, H, W = rotated.shape
                            rotated = logpolar_manual(rotated, (H, W), (H, W))

                            out_img = TF.to_pil_image(rotated.clamp(0, 1))
...
if __name__ == "__main__":
    Path(processed_dir:="processed_LPNET_dataset_1").mkdir(exist_ok=True)
    if len(sys.argv) > 1:
