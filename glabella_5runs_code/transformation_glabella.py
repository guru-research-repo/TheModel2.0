...
def get_glabella_or_center(image_bgr: np.ndarray) -> tuple:
    """
    Returns glabella (x, y) in pixels if detected, else image center.
    Input must be in BGR format (as read by OpenCV).
    """
    ...
    if results.multi_face_landmarks:
        glabella = results.multi_face_landmarks[0].landmark[168]
        return int(glabella.x * w), int(glabella.y * h)
    else:
        return w // 2, h // 2
